...
function LinkRow({ link }: { link: LinkType }) {
    ...
    const shortUrl = `${window.location.protocol}//${window.location.host}/${link.shortCode}`;
    ...
    return (
        <div className="grid grid-cols-12 gap-4 p-4 items-center text-sm hover:bg-white/5 transition-all group">
            <div className="col-span-5 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                    <div className="h-2 w-2 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" />
                    <a href={shortUrl} target="_blank" className="font-mono text-emerald-400 hover:underline truncate">
                        /{link.shortCode}
                    </a>
                </div>
                <div className="text-xs text-gray-500 truncate pl-4" title={link.originalUrl}>
                    {link.originalUrl}
                </div>
            </div>
            ...
            <div className="col-span-3 flex flex-col items-center justify-center">
                <div className="flex items-center gap-1.5 font-bold text-white bg-emerald-500/10 px-2 py-1 rounded border border-emerald-500/20">
                    {link.earnings.toFixed(4)} <span className="text-[10px] text-emerald-500">PLN</span>
                </div>
            </div>
            ...
            <div className="col-span-2 flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                >
                    {copied ? <span className="text-xs text-green-500 font-bold">✓</span> : <Copy size={16} />}
                </Button>
